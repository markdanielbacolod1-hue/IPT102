...
export function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (data.success) {
        setStats(data.stats);
        setUploadData(data.uploadsByMonth);
  ...
  return (
    <div className="p-6 bg-[url('./assets/cover.jpg')] bg-left bg-cover flex flex-col gap-6 h-full">
      ...
      <div className="grid grid-cols-4 gap-6">
        <StatCard icon={Users}    label="Total Users"       value={stats?.totalUsers    ?? 0} color="#768040" />
        <StatCard icon={FileText} label="Total Documents"   value={stats?.totalDocs     ?? 0} color="#408055" />
        <StatCard icon={Share2}   label="Shared Documents"  value={stats?.sharedDocs    ?? 0} color="#406480" />
        <StatCard icon={QrCode}   label="QR Scans"          value={stats?.qrScans       ?? 0} color="#6B4080" />
      </div>
  ...
  return (
    <div className="bg-white rounded-[10px] shadow-md relative overflow-hidden h-[112px] flex flex-col justify-center px-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-lg font-instrument text-black">{label}</p>
          <p className="text-[21px] font-medium font-instrument text-black">{value.toLocaleString()}</p>
        </div>
      </div>
      <div className="absolute bottom-0 left-0 w-full h-[8px]" style={{ backgroundColor: color }} />
    </div>
  );
}
