...
import { FileText, Users, ChevronDown, Bell, MessageSquare, Share2, Clock, X, Check, Send } from 'lucide-react';
...
export function DocumentSharing() {
  ...
  async function fetchDocuments() {
    const res  = await fetch(`${API}/sharing/documents`, { credentials: 'include' });
    const data = await res.json();
    setDocuments(data.documents || []);
  }

  async function fetchDepartments() {
    const res  = await fetch(`${API}/users/departments`, { credentials: 'include' });
    const data = await res.json();
    setDepartments(data.departments || []);
  }

  async function fetchRecentShares() {
    const res  = await fetch(`${API}/sharing/recent`, { credentials: 'include' });
    const data = await res.json();
    setRecentShares(data.shares || []);
  }

  async function fetchNotifications() {
    const res  = await fetch(`${API}/sharing/notifications`, { credentials: 'include' });
    const data = await res.json();
  ...
  async function handleMarkRead(id) {
    await fetch(`${API}/sharing/notifications/${id}/read`, {
      method: 'PATCH',
      credentials: 'include',
    });
  ...
  async function handleDismiss(id) {
    await fetch(`${API}/sharing/notifications/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    });
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <div
                  onClick={() => { setDocDropdown(!docDropdown); setDeptDropdown(false); }}
                ...
                {docDropdown && documents.length > 0 && (
                  <div className="absolute top-full left-0 right-0 z-20 bg-white border border-black rounded-[10px] shadow-xl mt-1 overflow-hidden max-h-60 overflow-y-auto">
                    {documents.map(doc => (
                      <div
                        key={doc.document_id}
                        onClick={() => { setSelectedDoc(doc.document_id); setDocDropdown(false); }}
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <div
                  onClick={() => { setDeptDropdown(!deptDropdown); setDocDropdown(false); }}
                  className="w-full h-[70px] rounded-[20px] border border-black bg-[#FFFCFC] shadow-md flex items-center px-6 cursor-pointer hover:bg-gray-50"
                >
                  ...
                  <div className="absolute top-full left-0 right-0 z-20 bg-white border border-black rounded-[10px] shadow-xl mt-1 overflow-hidden">
                    {departments.map(dept => (
                      <div
                        ...
                        onClick={() => { setSelectedDept(dept.department_id); setDeptDropdown(false); }}
            ...
            <div className="bg-gray-100 rounded-[10px] p-4 flex justify-end shadow-md">
              <button
                ...
                className={`h-[44px] px-6 rounded-[10px] shadow-md text-xl font-medium transition-all flex items-center gap-2 ${
                  sendStatus === 'sent'
                    ? 'bg-green-500 text-white'
                    : sendStatus === 'sending'
                    ? 'bg-gray-300 cursor-not-allowed text-black'
                    : !selectedDoc || !selectedDept
                ...
                <div key={share.share_id} className="w-full min-h-[70px] rounded-[20px] border border-black bg-[#FFFCFC] shadow-md flex items-center px-6 gap-4">
                  ...
                  <div className="flex-1">
                    <span className="text-base font-medium text-black">
                      {share.doc} → {share.recipient_dept}
                    </span>
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map(notif => (
              ...
              >
                <Share2 className="w-8 h-8 text-black shrink-0" />
                <div className="flex-1">
                  <span className={`font-medium text-black text-base ${!notif.is_read ? 'font-semibold' : ''}`}>
                    {notif.message}
                  </span>
